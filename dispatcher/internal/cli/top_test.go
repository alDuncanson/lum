...
func TestApplySnapshotUpdatesGaugeFields(t *testing.T) {
	...
	m.apply(events.Event{
		Kind: events.KindSnapshot, WorkerState: "ready", Detail: "model=x",
	...
	if m.workerState != "ready" || m.sources != 2 || m.documents != 5 || m.chunks != 40 || m.ingestFailures != 1 {
...
func TestApplyWorkerStateChangedUpdatesStateOutsideSnapshot(t *testing.T) {
	var m topModel
	m.apply(events.Event{Kind: events.KindWorkerStateChanged, FromState: "ready", WorkerState: "idle", Detail: "shed"})
	if m.workerState != "idle" || m.workerDetail != "shed" {
		t.Fatalf("state change not applied: %+v", m)
	}
	if len(m.recent) != 1 || m.recent[0].Kind != events.KindWorkerStateChanged {
...
func TestViewRendersWithoutPanicBeforeAndAfterEvents(t *testing.T) {
	...
	m.apply(events.Event{
		Kind: events.KindSnapshot, WorkerState: "ready", Sources: 1, Documents: 2, Chunks: 3,
	})
	...
	if !strings.Contains(out, "ready") || !strings.Contains(out, "boom") {
		t.Fatalf("view after events = %q, want worker state and last error rendered", out)
	}
}
