...
func (c *Client) ensureDaemon(ctx context.Context) error {
	...
	} else if state == string(dataplane.StateReady) {
		return nil
	}

	// Unavailable includes the short interval before lumen binds its socket,
...
func (c *Client) waitReady(ctx context.Context) error {
	...
	for {
		...
		if err == nil && state == string(dataplane.StateReady) {
			return nil
		}
		if err == nil {
...
func (c *Client) daemonStatus(ctx context.Context) (string, string, error) {
	...
	return status.DataPlane, status.Detail, nil
}
