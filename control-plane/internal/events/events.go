...
const (
	...
	// KindDataPlaneStateChanged fires on every lumen readiness transition
	// (ready, idle-shed, restarting, ...), observed from the periodic
	// snapshot loop polling dataplane.Manager.Health.
	KindDataPlaneStateChanged Kind = "dataplane_state_changed"

	// KindRPCCompleted covers whole-RPC latency for both transports: the
	// data-plane gRPC hop (Transport "grpc") and the public HTTP API
	// (Transport "http"). The control plane doesn't reach into lumen for
...
type Event struct {
	...
	// dataplane_state_changed and snapshot.
	DataPlaneState string `json:"data_plane_state,omitempty"`
